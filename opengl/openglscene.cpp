...
OpenGLScene::OpenGLScene()
    ...
    , m_modelColor(153, 155, 99) // 153, 255, 0
...
/*    QWidget *gcodeSlider = createDialog(tr("G-Code Slider"));
    ...
    // ================= Merge dialogs ==================
//    QWidget *widgets[] = { controls}; //, gcodeSlider , statistics
...
*/
    ...
    workArea ={DEFAULT_XWIDTH,DEFAULT_YDEPTH,DEFAULT_ZHEIGHT};
...
QDialog *
OpenGLScene::createDialog(const QString &windowTitle) const
{
...
QSlider *
OpenGLScene::createSlider(int rangeMax, const char *setterSlot)
{
...
QHBoxLayout *
OpenGLScene::createSpinBox(QString label,int rangeFrom, int rangeTo, const char *member)
{
...
QHBoxLayout *
OpenGLScene::createDoubleSpinBox(QString label, double rangeFrom, double rangeTo, double singleStep, const char *member)
{
    QLabel * spinLabel = new QLabel(label);
...
void
OpenGLScene::generateTube(QVector3D p1, QVector3D p2, QVector3D p3, float radius, bool saveRearVector)
{
...
void
OpenGLScene::drawBackground(QPainter *painter, const QRectF &)
{
    ...
    gluPerspective(70, width() / height(), 0.01, 1000);// 70;1000
...
//        glVertex3f(-GRID_SIZE, -GRID_SIZE, GRID_SIZE);
//        glVertex3f( GRID_SIZE, -GRID_SIZE, GRID_SIZE);
        glVertex3f(workArea.x_width/2.0f,workArea.y_depth/2.0f,workArea.z_height);
        glVertex3f(-workArea.x_width/2.0f,workArea.y_depth/2.0f,workArea.z_height);
        glVertex3f(-workArea.x_width/2.0f,-workArea.y_depth/2.0f,workArea.z_height);
        glVertex3f(workArea.x_width/2.0f,-workArea.y_depth/2.0f,workArea.z_height);
...
#if AXIS_UP==Z
//        glVertex3f( GRID_SIZE, GRID_SIZE,  GRID_SIZE);
//        glVertex3f( GRID_SIZE, GRID_SIZE,  0.f);
//        glVertex3f(-GRID_SIZE, GRID_SIZE,  GRID_SIZE);
//        glVertex3f(-GRID_SIZE, GRID_SIZE,  0.f);
//        glVertex3f(-GRID_SIZE, -GRID_SIZE, GRID_SIZE);
//        glVertex3f(-GRID_SIZE, -GRID_SIZE, 0.f);
...
//        glVertex3f( GRID_SIZE, -GRID_SIZE, 0.f);
        glVertex3f(workArea.x_width/2.0f,workArea.y_depth/2.0f,workArea.z_height);
        glVertex3f(workArea.x_width/2.0f,workArea.y_depth/2.0f,0.f);
        glVertex3f(-workArea.x_width/2.0f,workArea.y_depth/2.0f,workArea.z_height);
        glVertex3f(-workArea.x_width/2.0f,workArea.y_depth/2.0f,0.f);
        glVertex3f(-workArea.x_width/2.0f,-workArea.y_depth/2.0f,workArea.z_height);
        glVertex3f(-workArea.x_width/2.0f,-workArea.y_depth/2.0f,0.f);
...
void
OpenGLScene::drawAxis()
{
    QVector3D axispos(-50.0f,-50.0f,5.0f);
...
#endif
#if AXIS_UP==Z
        glVertex3f(axispos.x(), axispos.y(), axispos.z());
        glVertex3f(axispos.x()+ AXIS_SIZE, axispos.y(), axispos.z());
#endif
        // y
        glColor3f(0, 1, 0);
#if AXIS_UP==Y
...
#if AXIS_UP==Z
        glVertex3f(axispos.x(), axispos.y(), axispos.z());
        glVertex3f(axispos.x(), axispos.y() + AXIS_SIZE, axispos.z());
#endif
        // z
        glColor3f(0, 0, 1);
#if AXIS_UP==Y
...
#if AXIS_UP==Z
        glVertex3f(axispos.x(), axispos.y(), axispos.z());
        glVertex3f(axispos.x(), axispos.y(), axispos.z() + AXIS_SIZE);
#endif
    glEnd();
    glLineWidth(1);
...
{
/*    GLint param[256];
    glGetIntegerv(GL_ALIASED_LINE_WIDTH_RANGE,param);
    qDebug()<<__FILE__<<__LINE__<<param[2];
*/
    // disable lighting
    glDisable(GL_LIGHTING);
...
//    glVertex3f( GRID_SIZE, -GRID_SIZE, 0.f);
    ...
    for(float i=GRID_STEP; i <= workArea.x_width/2.0f; i+= GRID_STEP)
    {
#if AXIS_UP==Y
...
#endif
#if AXIS_UP==Z
//        glVertex3f(-GRID_SIZE, i,  0.f);   // lines parallel to X-axis
//        glVertex3f( GRID_SIZE, i,  0.f);
//        glVertex3f(-GRID_SIZE, -i, 0.f);   // lines parallel to X-axis
//        glVertex3f( GRID_SIZE, -i, 0.f);


//        glVertex3f( i, -GRID_SIZE, 0.f);   // lines parallel to Y-axis
//        glVertex3f( i, GRID_SIZE,  0.f);
//        glVertex3f(-i, -GRID_SIZE, 0.f);   // lines parallel to Y-axis
//        glVertex3f(-i, GRID_SIZE,  0.f);
        glVertex3f(i,-workArea.y_depth/2.0f,0.f);
        glVertex3f(i,workArea.y_depth/2.0f,0.f);
        glVertex3f(-i,-workArea.y_depth/2.0f,0.f);
        glVertex3f(-i,workArea.y_depth/2.0f,0.f);
...
#if AXIS_UP==Z
    for(float i=GRID_STEP; i <= workArea.y_depth/2.0f; i+= GRID_STEP)
    {
        glVertex3f(-workArea.x_width/2.0f,i ,0.f);
        glVertex3f(workArea.x_width/2.0f,i, 0.f);
        glVertex3f(-workArea.x_width/2.0f,-i ,0.f);
        glVertex3f(workArea.x_width/2.0f,-i, 0.f);
    ...
    glColor3f(1, 0, 0);
#if AXIS_UP==Y
    glVertex3f(-GRID_SIZE, 0.f, 0.f);
    glVertex3f( GRID_SIZE, 0.f, 0.f);
#endif
#if AXIS_UP==Z
//    glVertex3f(-GRID_SIZE, 0, 0);
//    glVertex3f( GRID_SIZE, 0, 0);
    glVertex3f(-workArea.x_width/2.0f,0.f ,0.f);
    glVertex3f(workArea.x_width/2.0f,0.f, 0.f);
...
#endif
#if AXIS_UP==Z
//    glVertex3f(0, -GRID_SIZE, 0.f);
//    glVertex3f(0, GRID_SIZE,  0.f);
    glVertex3f(0.0f,-workArea.y_depth/2.0f ,0.f);
    glVertex3f(0.0f,workArea.y_depth/2.0f , 0.f);
...
void
OpenGLScene::enableWireframe(bool enabled)
{
...
void
OpenGLScene::enableNormals(bool enabled)
{
...
void
OpenGLScene::enableGCodeMotion(bool enabled)
{
...
void
OpenGLScene::enableGCodeLines(bool enabled)
{
...
void
OpenGLScene::setModel(Model *model)
{
    delete m_model;
    m_model = model;
#if EMBED==0
...
#endif
#if EMBED==1
    emit modelFileOpened(m_model);
#endif
    update();
}

QColor
OpenGLScene::getModelColor() const
{
    return m_modelColor;
}

void
OpenGLScene::setModelColor(const QColor color)
{
    if(color.isValid()){
        m_modelColor = color;
        update();
...
void
OpenGLScene::setModelWorkArea(const ModelWorkArea &value)
{
    workArea = value;
}
...
QColor
OpenGLScene::getBackGroundColor() const
{
    return m_backgroundColor;
}

void
OpenGLScene::setBackgroundColor_ex(const QColor color)
{
    if(color.isValid()){
