    ...
    : m_fileName(QFileInfo(filePath).fileName())
{
    ...
    if (filePath.endsWith(".stl", Qt::CaseInsensitive)) {
        loadStl(file);
    } else if(filePath.endsWith(".ast",Qt::CaseInsensitive)){
        loadStl(file);
...
//            stream >> n[0] >> n[1] >> n[2];
            stream>>word;
            GLfloat nx = QString(word).toFloat(&ok);
            stream>>word;
            GLfloat ny = QString(word).toFloat(&ok);
            stream>>word;
            GLfloat nz = QString(word).toFloat(&ok);
...
//                stream >> v[0] >> v[1] >> v[2];
                stream>>word;
                GLfloat vx = QString(word).toFloat(&ok);
                stream>>word;
                GLfloat vy = QString(word).toFloat(&ok);
                stream>>word;
                GLfloat vz = QString(word).toFloat(&ok);
                QVector3D v(vx,vy,vz);
                m_vertices.push_back(v);
//                qDebug()<<__FILE__<<__LINE__ << "==== outer loop ===";
            }
/*
            for(size_t i = startIndex + 2 ; i < m_vertices.size() ; ++i)
            {
                ...
                qDebug()<<__FILE__<<__LINE__ << QString("edgeA(%1), edgeB(%2)").arg(QString::number(startIndex), QString::number(i)) ;

//                if (startIndex < (i-1))
                    m_edgeIndices << static_cast<int>(startIndex) << static_cast<int>(i - 1) << static_cast<int>(i);
            }
*/
