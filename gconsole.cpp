...
GConsole::setupGconsole()
{
    ...
    if(cord->isAbsolute())
    {
        ui->label_AbsRel->setText(MyGlobal::msg_absolute);
        ui->label_AbsRel->setToolTip(MyGlobal::msg_abs_title);
    }else{
        ui->label_AbsRel->setText(MyGlobal::msg_relative);
        ui->label_AbsRel->setToolTip(MyGlobal::msg_rel_title);
    }
...
//    ui->dockWidget_Console->setCursor(Qt::BusyCursor);

}

void GConsole::setEnabledCursor() {
    ui->pushButton_linestep->setDisabled(false);
//    ui->dockWidget_Console->setCursor(Qt::ArrowCursor);
}
...
//    qDebug()<<"GConsole[60] Clicked line:"<<bnumber ;
    ...
    QString block =  ui->textEdit_command->textCursor().block().text();
...
//    uia->label_commandLine->setText(uia->textEdit_command->document()->findBlockByLineNumber(bnumber).text());
    ui->label_commandLine->setText(pbuffer);
// -------- parcer
...
{
    int bnumber = ui->textEdit_command->textCursor().block().blockNumber();

    QColor bkgColor(170, 255, 255);

//    QTextFormat format = ui->textEdit_command->textCursor().block().blockFormat();
    QString str = ui->textEdit_command->textCursor().block().text();
...
//    ui->textEdit_command->textCursor().block().blockFormat().setBackground(QBrush(QColor(120,120,120)));
    ...
    QTextBlock block =  ui->textEdit_command->textCursor().block();
    ...
    if(oldBlockNumber>=0 && oldBlockNumber!= bnumber){
        QTextBlock oldBlock = ui->textEdit_command->document()->findBlockByLineNumber(oldBlockNumber);
